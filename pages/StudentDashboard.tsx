            ...
            <div className="w-20 h-20 md:w-24 md:h-24 bg-white rounded-[2rem] flex items-center justify-center overflow-hidden border-4 border-white shadow-xl transform -rotate-3 hover:rotate-0 transition-transform shrink-0">
              {isAvatarUrl ? (
                <img 
                  ...
                  key={currentUser?.avatar} // Force refresh if image string changes
                  alt="Profile" 
            ...
            <div className="space-y-3">
              {filteredChapters.length > 0 ? filteredChapters.map((chapter: Chapter) => (
                <div key={chapter.id} className="p-5 bg-white rounded-[1.5rem] shadow-md border-b-4 border-slate-200 hover:border-sky-300 transition-all hover:-translate-y-1">
                  <span className="text-xs font-black text-sky-500 uppercase tracking-widest">Ongoing</span>
                  <h3 className="font-black text-slate-800 mt-1 text-lg leading-tight">{chapter.name}</h3>
                </div>
              )) : (
                <div className="p-6 bg-white rounded-3xl border-4 border-dashed border-slate-100 text-center text-slate-300">
                    ...
                    <div className="aspect-video bg-slate-800 flex items-center justify-center text-white relative">
                      ...
                      {currentUser?.progress.watchedVideos.includes(video.id) && (
                        <div className="absolute top-4 right-4 bg-green-500 text-white p-2 rounded-2xl shadow-lg border-2 border-white flex items-center space-x-1">
                          ...
                          <span className="text-xs font-black">WATCHED</span>
                        </div>
                      )}
                    </div>
                    <div className="p-6">
                      <h3 className="text-2xl font-black text-slate-800 leading-tight">{video.title}</h3>
                      <p className="text-slate-500 mt-2 font-medium">Click to start the lesson!</p>
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {content.maps.length > 0 ? content.maps.map((map: MindMap) => (
                  <div key={map.id} className="bg-white rounded-[2.5rem] p-8 shadow-xl border-4 border-white hover:shadow-2xl transition-all text-center">
                    ...
                    <p className="text-slate-500 font-bold mb-6">{map.type === 'pdf' ? 'Printable PDF Resource' : 'Educational Chart'}</p>
                ...
                {content.quizzes.length > 0 ? content.quizzes.map((quiz: Quiz) => (
                  <div key={quiz.id} className="bg-white rounded-[3rem] p-8 shadow-xl flex flex-col md:flex-row items-center justify-between border-4 border-white hover:shadow-2xl transition-all">
                    <div className="flex items-center space-x-8">
                      <div className="w-20 h-20 bg-amber-100 rounded-[2rem] flex items-center justify-center text-4xl shadow-inner shrink-0">
                        📝
                      </div>
                      <div>
                        <h3 className="text-3xl font-black text-slate-800 leading-tight">{quiz.title}</h3>
                        <p className="text-slate-500 font-bold mt-1 uppercase text-xs tracking-widest">{quiz.questions.length} Questions • Big Challenge</p>
                      </div>
                    </div>
                    <div className="mt-6 md:mt-0 flex items-center space-x-6">
                      {currentUser?.progress.quizScores[quiz.id] !== undefined && (
                        <div className="text-center bg-slate-50 px-6 py-3 rounded-3xl border-2 border-slate-100">
                          <span className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">BEST SCORE</span>
                      ...
                      >
                        {currentUser?.progress.quizScores[quiz.id] !== undefined ? 'Retake it!' : 'Go for it! 🎯'}
                      </button>
                    </div>
                  </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
            <div className="bg-white w-full max-w-4xl rounded-[3rem] overflow-hidden shadow-2xl relative">
              <button 
                ...
                className="absolute top-6 right-6 z-10 w-12 h-12 bg-black/40 hover:bg-black/60 text-white rounded-full flex items-center justify-center text-2xl transition-colors border-2 border-white/50"
              >
                ✕
              </button>
              ...
              <div className="p-8">
                ...
                <p className="text-sky-500 font-bold mt-2 flex items-center">
                  <span className="mr-2">✨</span>
                  Watching this helps you level up your progress!
                </p>
              </div>
            </div>
          </div>
        ...
        {quizScore && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <div className="bg-white w-full max-w-md rounded-[4rem] p-12 text-center shadow-2xl">
              ...
              <h2 className="text-5xl font-black text-slate-800 mb-4">Super Star!</h2>
              <p className="text-2xl text-slate-500 font-bold mb-10">
                You scored <span className="text-sky-600 font-black">{quizScore.score}</span> / <span className="font-black">{quizScore.total}</span>!
              </p>
              <button 
                onClick={() => { setQuizScore(null); setActiveQuiz(null); }}
                className="w-full bg-sky-500 hover:bg-sky-600 text-white font-black py-6 rounded-[2.5rem] shadow-xl transition-all hover:scale-105 active:scale-95 text-3xl"
              >
                Let's Go! 🎉
              </button>
            </div>
      ...
      <div className="bg-white w-full max-w-2xl rounded-[3.5rem] p-10 shadow-2xl relative">
        ...
        <div className="mb-10">
          <div className="flex items-center justify-between mb-4">
            <span className="text-xs font-black text-sky-500 uppercase tracking-[0.2em]">QUESTION {currentIdx + 1} OF {quiz.questions.length}</span>
            <div className="h-4 w-40 bg-slate-100 rounded-full overflow-hidden border-2 border-slate-50">
              <div className="h-full bg-sky-500 transition-all duration-500" style={{ width: `${((currentIdx + 1) / quiz.questions.length) * 100}%` }}></div>
            </div>
          </div>
        ...
        <div className="space-y-4 mb-12">
          {quiz.questions[currentIdx].options.map((opt, i) => (
            <button
              ...
              className={`w-full p-6 text-left rounded-[2rem] border-4 font-black text-xl transition-all ${
                answers[currentIdx] === i 
                  ? 'bg-sky-50 border-sky-400 text-sky-700 shadow-lg scale-[1.02]' 
                  : 'bg-white border-slate-100 hover:border-slate-200 text-slate-600'
              }`}
            >
              <span className={`inline-block w-10 h-10 rounded-2xl mr-4 text-center leading-10 font-black ${answers[currentIdx] === i ? 'bg-sky-500 text-white' : 'bg-slate-100'}`}>
        ...
        <div className="flex space-x-6">
          <button 
            ...
            className="flex-1 py-5 bg-slate-100 text-slate-400 font-black rounded-[1.5rem] disabled:opacity-30 transition-all text-lg"
          >
            Previous
          </button>
          <button 
            disabled={answers[currentIdx] === undefined}
            onClick={() => isLast ? onSubmit(answers) : setCurrentIdx(prev => prev + 1)}
            className={`flex-1 py-5 text-white font-black rounded-[1.5rem] shadow-xl transition-all text-lg ${
              answers[currentIdx] === undefined ? 'bg-slate-300 cursor-not-allowed' : 'bg-sky-500 hover:bg-sky-600 hover:scale-105 active:scale-95'
            }`}
          >
...
const EmptyState: React.FC<{ icon: string, message: string }> = ({ icon, message }) => (
  <div className="col-span-full py-24 bg-white rounded-[4rem] border-4 border-dashed border-slate-100 text-center shadow-inner">
    ...
    <p className="text-slate-200 font-bold mt-2">Check back again soon!</p>
  </div>
);
